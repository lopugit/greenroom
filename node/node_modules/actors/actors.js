				...
				for await(var i of loop){
					let actor = {
						...
						available: async function (args){
							var actor = this
							try {
								for (let page of Object.keys(actor.pages)) {
									try {
										await actor.pages[page].close()
									} catch (err) {
										global.logger.error("[greenroom][actors][available] Page close errored", err)
									}
								}
							} catch (err) {
								global.logger.error("[greenroom][actors][available] Pages close errored", err)
							}
						...
						pageCrash: async function () {
							...
							try {
								...
								global.logger.error("[greenroom][puppeteer][browser][error] Launching new browser")
							}
