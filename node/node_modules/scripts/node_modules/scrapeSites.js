...
module.exports = {
	...
	run: async function (args){
		let puppets = await require('puppets')()
		try {
				...
				for await(let site of sites) {
					try {
						...
						for (let pageId of pageIds){
							try {
								let puppet = await puppets.available()
								let url = pages[pageId].createUrl({self: pages[pageId]})
								puppet.page = await puppet.getBrowser().newPage()
								// fake multi threading
								let promise = puppet.page.goto(url, { timeout: 1000*15 })
								.then(async ()=>{
									try {
										await puppet.page.waitForSelector(pages[pageId].list.selector+" > *", { timeout: 1000*2 })
										let els = await puppet.page.$$(pages[pageId].list.selector+" > *")
										if(!els || !els.length){
											...
											for await(let el of els){
												try {
													...
													global.logger.log(`[greenroom][scrapeSites][info] Using collection ${pages[pageId].schema.path}`)
													...
													await scrapeProperties({properties, obj, puppet, model, schema, el})
								...
								.finally(()=>{
									global.logger.log("[greenroom][scrapeSites][info] Making puppet available again")
									puppet.available()
									puppet.page.close()
								})
								promises.push(
			...
			console.error("[greenroom][scrapeSites][error] ", err)
		} finally {
			puppets.close()
		}
	}
...
async function getVal(args){
	...
	if(typeof path !== 'undefined' && args.model){
		...
		try {
			...
			global.logger.error("[greenroom][error][scrapeSites][getVal]", err)
		}
	}
	return
}