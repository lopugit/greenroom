					...
					try {
						...
						for (let pageId of pageIds){
							try {
								...
								actor.page = await actor.getBrowser().newPage()
								// fake multi threading
								let promise = actor.page.goto(url, { timeout: 1000*15 })
								.then(async ()=>{
									try {
										await actor.page.waitForSelector(pages[pageId].list.selector+" > *", { timeout: 1000*2 })
									} catch (err) {
										global.logger.error("[greenroom][scrapeSites][waitForSelector] Waiting for selector failed", err)
										return
									}
									try {
										let els = await actor.page.$$(pages[pageId].list.selector+" > *")
										if(!els || !els.length){
											...
											for await(let el of els){
												try {
													...
													await scrapeProperties({properties, obj, actor, model, schema, el})
