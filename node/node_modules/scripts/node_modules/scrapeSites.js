...
global.logger.log("[muppets][scrapeSites][info] Connecting via monk to URI "+uri)
...
module.exports = {
	...
	run: async function (args){
		let muppets = await require('muppets')()
		if(muppets){
			global.logger.log(`[muppets][scrapeSites][info] Starting scrape at ${Date()}`)
			let time = DateTime.now()
			let processTimer = process.hrtime()
			let totalProducts = 0
			let promises = []
			try {
				global.logger.log(`[muppets][scrapeSites][info] Scraping ${sites.length} sites`)
				for await(let site of sites) {
					try {
						global.logger.log(`[muppets][scrapeSites][info] Scraping ${site.name}`)
							...
							try {
								...
								.then(async ()=>{
									try {
										...
										if(!els || !els.length){
											global.logger.log('[muppets][scrapeSites][info] No products to scrape')
										} else {
											for await(let el of els){
												try {
													...
													global.logger.log(`[muppets][scrapeSites][info] Using collection ${pages[pageId].schema.path}`)
													...
													global.logger.log("[muppets][scrapeSites][info] Validating newObj")
													...
													global.logger.log(`[muppets][scrapeSites][info] Scraped ${updated.title}`)
													global.logger.log(`[muppets][scrapeSites][info] From ${updated.source}`)
													global.logger.log(`[muppets][scrapeSites][info] To ${schema.path}`)
													totalProducts++
												} catch (err) {
													global.logger.error("[muppets][scrapeSites][error]", err)
												}
											}
										}
									} catch (err) {
								...
								.finally(()=>{
									global.logger.log("[muppets][scrapeSites][info] Making muppet available again")
									muppet.available()
									muppet.page.close()
								})
								promises.push(
				...
				global.logger.log("[muppets][scrapeSites][summary][info] Done! Took", finishTime[0], "seconds. Scraped", totalProducts, "products")
			}
		}
	}
}
...
async function getVal(args){
	...
	if(typeof path !== 'undefined' && args.model){
		...
		try {
			...
			global.logger.error("[muppets][error][scrapeSites][getVal]", err)
		}
	}
	return
}