	...
	let muppets = {
		...
		async "find muppets"(args){
			if(typeof args.number !== undefined && typeof args.number == 'number' && !this.finding){
				rollbar.log(`finding ${args.number} ${args.number == 1 ? 'muppet' : 'muppets'}`)
				this.finding = true
		...
		log(args){
			...
			this.log.loop = this.log.loop || setInterval(() => {
				if(prevmuppets !== this["ready"].length){
					rollbar.log(`${this["ready"].length} ${this["ready"].length == 1 ? 'muppet' : 'muppets'} ready, ${Object.keys(this["running"]).length} running, ${this.muppets.length} in total.`)
